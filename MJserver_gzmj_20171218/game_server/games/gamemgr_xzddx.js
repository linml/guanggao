...
function doGameOver(game,userId,forceEnd){
    ...
    if(!forceEnd && game){
        ...
        if(roomInfo.numOfGames == 1){
            ...
            db.cost_gems(game.gameSeats[0].userId,cost, comdef.CASH_CHANGE_RESONS.COST_CREATE_ROOM.format(roomId));
        }
...
function store_single_history(userId,history){
    var data = db.get_user_history(userId);
    if(data == null){
        data = [];
    }
...
function store_game(game){
    return db.create_game(game.roomInfo.uuid,game.gameIndex,game.baseInfoJson);
}
