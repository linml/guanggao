// 东莞红中王麻将
var mjutils = require('./laizimjutils_hzw');
...
var myVersion = "dghzw V0.01 171215";
...
var ACTION_ZIMO = 6;
var ACTION_CHI = 7;
var ACTION_BUHUA = 8;   //mc buhua

var gameSeatsOfUsers = {};

var BAOLIU_PAICNT = 0;       //mc
...
//是否春夏秋冬  mc
function isCXQDType(id) {
    return getMJType(id) == 4;
}
// 是否梅兰竹菊  mc
function isMLZJType(id) {
    return getMJType(id) == 5;
}
...
// 判断是否是花牌
function isHua(pai){
    if (isCXQDType(pai) || isMLZJType(pai)){
        return true
    }
    return false;
}
...
function deal(game) {
    ...
    for (var i = 0; i < 13 * game.currentUsercnt; ++i) {
        var turnSeat = game.gameSeats[seatIndex]; //当前玩家座位信息
        var mahjongs = game.gameSeats[seatIndex].holds;
        if (mahjongs == null) {
            mahjongs = [];
            game.gameSeats[seatIndex].holds = mahjongs;
        }
        var paiMo = mopai(game, seatIndex);
        //发牌时判断摸的牌是否是花牌
        var huanPai = (function (paiMo) {
            if(isHua(paiMo)){
                buhua(game,turnSeat,paiMo);
                paiMo = mopai(game, seatIndex);
                //通知房间里对应的玩家补花
                userMgr.broacastInRoom('game_buhua_push',{huaIndex:paiMo,huamap:turnSeat.huaMap[i]},turnSeat.userId,true); //哪张牌和牌数量
                huanPai(paiMo);
            }
        })(paiMo);
        seatIndex++;
    ...
    //庄家多摸最后一张
    var paiMoButton = mopai(game, game.button);
    huanPai(paiMoButton);
    //当前轮设置为庄家
...
function checkCanHu(game, seatData, targetPai) {
    ...
    var huInfo = checkCanHuExt(game, seatData, targetPai, false);
    ...
    if (huInfo) {
        var tingInfo = {
            ...
            isMenQing: huInfo.isMenQing,   //mc
            is28Yan: huInfo.is28Yan,
            isZiYan: huInfo.isZiYan,
            isDuanYao: huInfo.isDuanYao,
            isQueYiMen: huInfo.isQueYiMen,
            isWuHua: huInfo.isWuHua,
            isPPHu: huInfo.isPPHu,
            isHunYiSe: huInfo.isHunYiSe,
            isQingYiSe: huInfo.isQingYiSe,
            isYaoJiuPai: huInfo.isYaoJiuPai,
            isDDQing: huInfo.isDDQing,
            isHaiDiLaoYue: huInfo.isHaiDiLaoYue,
...
var checkCanHuExt = function (game, seatData, chuPai, jingmode) {
    ...
    //     causePattern = "13lan";
    // }
    else if (isShiSanYao(seatData, jingmode, chuPai) > 0){
        causePattern = '13Yao';
    }
...
function scanPattern(game, seatData, chuPai) {
    ...
    //todo 扫描十三幺 mc
    mjutils.scan13YaoPattern(seatData, false, chuPai, patterns);
...
function jiSuanJingFen(game, seatData) {
    for (var i = 0; i < seatData.jingsInfo.length; i++) {
        ...
        if (oneJingFen >= 5) { // 是否冲关
            console.log("jiSuanJingFen 冲关成功.");
                    ...
                    for (var g = 0; g < game.allJings.length; g++) {
                        ...
                        for (var gc = 0; gc < oneJing.length; gc++) {
                            if (ac.pai == oneJing[gc]) {
                                ...
                                isGangJing = true;
                                gangBaseScore = 10;
                                // 判断下是不是霸王，假如是，则 gangBaseScore*2 ; //laoli 1128
                                var otherNotHave = true;
                                for (var bj = 0; bj < gameSeatLength; bj++) {  //laoli 1021 for 23mj
                                    if (i != bj) {
                                        var sdd = game.gameSeats[bj];
                                        if (sdd.jingsInfo[g].jiShu[0] > 0 || sdd.jingsInfo[g].jiShu[1] > 0) {
                                            otherNotHave = false;
                                            break;
                                        }
                                    }
                                ...
                                if (otherNotHave) {
                                    gangBaseScore = gangBaseScore * 2; // bawang for gangjing
                                    console.log("gangjing with bawang!")
                                }
                                if (g == 0) {
...
exports.begin = function (roomId) {
    ...
    notifyChuPai(game);
    //开局是检测玩家之间的距离开启防作弊通知 add gl 11/22
    //var n_data = {roomId: roomId};
    //http.get(roomInfo.GPS_USE_IP, roomInfo.GPS_USE_PORT, "/get_distance_two", n_data, function (ret, data) {
    //    console.log('获取gps数据完成！', data);
    //    if (ret && data && data.errcode && (data.errcode == 10000 || data.errcode == 10001 || data.errcode == 10002 || data.errcode == 10003)) {
    //        console.log("game.currentUsercnt>>", game.currentUsercnt);
    //        for (var i = 0; i < game.currentUsercnt; i++) {
    //            var s = seats[i];
    //            userMgr.sendMsg(s.userId, 'game_alert_msg', data.errmsg);
    //        }
    //    }
    // });  //laoli removed 171202 for gps not ready
};
...
exports.getConf = function (roomConf, creator) {
    console.log("getConf roomCnf:", roomConf);
    console.log("getConf creator:", creator);
    var ret = {
        type: roomConf.type,
...
isShiSanYao = function (seatData, checkJings, chupai) {
    var fn = function (seatData, jingMode) {
        //console.log(">> is13Yao:",seatData,chupai)
        var xucnt=0,laotoupaicnt=0;
        var singlecnt=0;
        ...
        for (var i in seatData.countMap) {
            ...
            if (isMJXuType(pai)){
                xucnt++;
                if(!isYaoJiu(pai)) return -1;
            }else if (isMJZiType(pai)){
                laotoupaicnt++;
            }
            else {
                //return -1;
        ...
        console.log("========is13Yao:",xucnt,laotoupaicnt,singlecnt)

        if (singlecnt!=12) return -1;
        if (xucnt < 6) return -1;
        if (laotoupaicnt<7) return -1;
        return 1;
    };
    ...
    return resultp(seatData, checkJings)
};
...
is6PairsEx = function (seatData) {
    ...
    console.log("is6PairsEx paiCount:", paiCount, "jingCount:", jingCount, "pairCount:", pairCount);
...
isJingDiaoPai = function (seatData, checkJings, chupai) {
    console.log("isJingDiaoPai", checkJings, chupai);
    var fn = function (seatData, jingMode, chupai) {
        var ret = 0;
        var oldCountMap = {};
        ...
        if (removeSuccess != true) {
            console.log("isJingDiaoPai removeSuccess:", removeSuccess);
            ret = -701;
        } else {
            if (is6PairsEx(seatData, -1)) {
                ...
                console.log("isJingDiaoPai", fnRet);
                ret = fnRet ? 0 : -702;
            }
        }
        // 恢复countMap
