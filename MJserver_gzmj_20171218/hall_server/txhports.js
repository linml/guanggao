...
exports.init = function (config, $gevent, configs) {
    ...
    ADDGEMS = DAI_LI.ADD_GEMS;//代理账号为代理游戏账号充值gems数量；
...
var creat_association = function (txh_name, txh_id, txh_quyu, txh_disc, target_account, creator_id, creator_name, creator_position, callback) {
    ...
    var txh_all_man = 200;
    var sql = 'INSERT INTO txh_info(txh_id, txh_name, txh_quyu, txh_all_man, txh_disc, target_account, creator_id, creator_name, creator_position) VALUES("{0}","{1}","{2}","{3}","{4}","{5}","{6}","{7}","{8}")';
    sql = sql.format(txh_id, txh_name, txh_quyu, txh_all_man, txh_disc, target_account, creator_id, creator_name, creator_position);
    query(sql, function (err, rows, fields) {
        if (!err) {
            var data = {'code': '100'};
            callback(data);
...
var add_active_info = function (txh_id, todo_user_account, about_user_account, todo_status, callback) {
    ...
    var sql = 'INSERT INTO txh_action_table(txh_id,todo_user_account,about_user_account,todo_status) VALUES("{0}","{1}","{2}","{3}")';
    sql = sql.format(txh_id, todo_user_account, about_user_account, todo_status);
    query(sql, function (err, rows) {
        if (!err) {
            var data = {'code': 100};
            callback(data);
...
var is_creat_txh = function (target_account, creator_id, callback) {
    ...
    try {
        ...
        email.send_email("创建牌友会调用/api/v1/dealers/checkdaili接口出错", "err" + e.stack);
        return;
    }
};
...
var txh_addnumbers = function (txh_id, user_id, target_account, user_name, user_title, apply_position, callback) {
    ...
    var sql1 = 'SELECT * FROM txh_member_table WHERE txh_id = "{0}" and user_id ="{1}"';
    sql1 = sql1.format(txh_id, user_id)
    query(sql1, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                ...
                var sql = 'INSERT INTO txh_member_table(txh_id,user_id,target_account,user_name,user_title,apply_position) VALUES("{0}","{1}","{2}","{3}",{4},"{5}")';
                sql = sql.format(txh_id, user_id, target_account, user_name, user_title, apply_position)
                query(sql, function (err, rows) {
                    if (!err) {
...
var del_user_info = function (txh_id, target_account, callback) {
    ...
    var sql = 'SELECT * FROM txh_info WHERE target_account = "{0}" and txh_id = "{1}" ';
    sql = sql.format(target_account, txh_id);
    query(sql, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                var data = {code: 120}
                ...
                var sql = 'SELECT * FROM txh_member_table WHERE target_account = "{0}" and txh_id = "{1}"';
                sql = sql.format(target_account, txh_id);
                query(sql, function (err, rows) {
                    if (!err) {
                        if (rows && rows[0]) {
                            var sql = 'delete from txh_member_table where txh_id = "{0}"and target_account = "{1}"';
                            sql = sql.format(txh_id, target_account);
                            query(sql, function (err, rows) {
                                if (!err) {
                                    // console.log(rows);
                                    var temp = 0;
...
var pdishuizhang = function (txh_id, target_account, callback) {
    ...
    var sql = 'SELECT user_title FROM txh_member_table WHERE target_account = "{0}" and txh_id = "{1}" ';
    sql = sql.format(target_account, txh_id);
    query(sql, function (err, rows) {
        if (!err) {
    ...
    query(sql1, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                ...
                var sql = 'update txh_info set txh_now_man = {0} where txh_id = "{1}"';
                sql = sql.format(txh_now_man, txh_id)
                query(sql, function (err, rows) {
                    if (!err) {
                        var data = {"code": 100};
                        callback(data);
...
var set_vicechairman = function (txh_id, target_account, callback) {
    ...
    var sql = 'SELECT user_title FROM txh_member_table WHERE txh_id = "{0}"and target_account = "{1}"';
    sql = sql.format(txh_id, target_account);
    query(sql, function (err, rows) {
        if (!err) {
                    ...
                    if (!err) {
                        ...
                        if (rows && rows.length < 2) {
                            ...
                            query(sql, function (err, rows) {
                                if (!err) {
                                    if (rows && rows[0]) {
                                        var sql1 = 'update txh_member_table set user_title = 112 where txh_id = "{0}"and target_account = "{1}"';
                                        sql1 = sql1.format(txh_id, target_account);
                                        query(sql1, function (err, rows) {
                                            if (!err) {
                                                var data = {'code': 100}
                                                callback(data);
        ...
        if (!err) {
            if (rows && rows[0]) {
                if (rows[0].user_title == 112) {//guolin 11/13 会长解除玩家职位时 如果是副会长才能操作
                    var sql1 = 'update txh_member_table set user_title = 113 where txh_id = "{0}"and target_account = "{1}"';
                    sql1 = sql1.format(txh_id, target_account)
                    query(sql1, function (err, rows) {
                        if (!err) {
                            var data = {'code': 100}
...
var txh_add_member = function (txh_id, target_account, callback) {
    ...
    var sql1 = 'SELECT * FROM txh_member_table WHERE target_account = "{0}" and user_title = 113';
    sql1 = sql1.format(account2)
    query(sql1, function (err, rows) {
        if (!err) {
            if (rows && rows.length < 4) {
                var sql = 'SELECT txh_now_man, txh_all_man FROM txh_info WHERE txh_id = "{0}"';
                sql = sql.format(txh_id)
                query(sql, function (err, rows) {
                    if (!err) {
...
var txh_all_member = function (txh_id, callback) {
    ...
    var sql = "SELECT * FROM txh_member_table WHERE txh_id = '{0}'";
    sql = sql.format(txh_id)
    //log.error('txh sql member >>>',sql);
    query(sql, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                var data = {"code": 100, 'rows': rows};
...
var hz_search_info = function (txh_id, deal_user_account, callback) {
    ...
    var sql = "SELECT * FROM txh_info_msg WHERE txh_id = '{0}'and deal_user_account = '{1}'and deal_status = 0";
    sql = sql.format(txh_id, account)
    query(sql, function (err, rows) {
        if (!err) {
...
var user_sign_out_txh = function (user_id, txh_id, callback) {
    ...
    query(sql1, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                var sql = 'delete from txh_member_table where txh_id = "{0}"and user_id ="{1}"';
                sql = sql.format(txh_id, user_id)
                query(sql, function (err, rows) {
                    if (!err) {
...
var hz_del_txh = function (txh_id, creator_id, callback) {
    ...
    var sql = 'SELECT * FROM txh_info WHERE txh_id = "{0}" and creator_id = "{1}"';
    sql = sql.format(txh_id, creator_id);
    query(sql, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                var t_sql = 'SELECT * FROM txh_room_table WHERE txh_id = "{0}" and room_is_excit = 0';//add11/13牌友会有房间为解散不让解散
                t_sql = t_sql.format(txh_id)
...
var change_txh_info = function (txh_id, txh_disc, creator_id, txh_name, callback) {
    ...
    var sql = "SELECT * FROM txh_info WHERE txh_id = '{0}'and creator_id ='{1}'"; //laoli 1202
    sql = sql.format(txh_id, creator_id)
    query(sql, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                // log.error('hui zhang shu ju >>', rows);
                var sql1 = 'update txh_info set txh_disc = "{0}",txh_name = "{1}" where txh_id = "{2}"and creator_id = "{3}"';
                sql1 = sql1.format(txh_disc, txh_name, txh_id, creator_id);
                query(sql1, function (err, rows) {
                    if (!err) {
                        var data = {"code": 100}
                        callback(data)
...
var txh_create_room = function (room_id, txh_id, creator_id, callback) {
    ...
    var room_is_excit = 0;
    var sql = 'INSERT INTO txh_room_table (room_id,txh_id,creator_id,room_is_excit) VALUES ("{0}","{1}","{2}",{3})';
    sql = sql.format(room_id, txh_id, creator_id, room_is_excit);
    query(sql, function (err, rows) {
        if (!err) {
            var data = {"code": 100};
            callback(data)
...
//牌友会成员活跃积分排名；add gl 12/15
function menber_rink(txh_id,time,iftrue,callback){
    var sql = "select game_index,result,uuid,user_id0,user_id1,user_id2,user_id3,room_id,txh_id from" +
        " (select * from t_rooms_archive inner join txh_room_table on t_rooms_archive.id = txh_room_table.room_id where" +
        " txh_room_table.txh_id = '"+txh_id+"' and date_format(from_unixtime(t_rooms_archive.finish_time),'%Y%m%d') = " +
        " DATE_FORMAT(date_add(CURDATE(), interval "+time+" day),'%Y%m%d')) as d inner join" +
        " t_games_archive on d.uuid = t_games_archive.room_uuid";
    query(sql,function(err,rows){
        if(!err){
            var js_data = rows;
            var sql1 = 'SELECT * FROM txh_member_table where txh_id = "'+txh_id+'"';
            query(sql1,function(err,rows){
                if(!err){
                    console.log('txh_menber data>>',rows);
                    var men_data = rows;
                    for(var i = 0; i< men_data.length;i++){
                        men_data[i].temp = 0;
                        men_data[i].user_jf = 0;
                        var user_id = men_data[i].user_id;
                        for(var j = 0;j< js_data.length; j ++){
                            var num = [];
                            if(js_data[j].result){
                                num = JSON.parse(js_data[j].result);
                            }
                            if(js_data[j].user_id0 == user_id){
                                men_data[i].temp ++;
                                if(num[0]){
                                    if(num[0] >= 0){
                                        men_data[i].user_jf += 5 + num[0];
                                    }else {
                                        men_data[i].user_jf += 5
                                    }
                                }
                            }else if(js_data[j].user_id1 == user_id){
                                men_data[i].temp ++;
                                if(num[1]){
                                    if(num[1] >= 0){
                                        men_data[i].user_jf += 5 + num[1];
                                    }else {
                                        men_data[i].user_jf += 5
                                    }
                                }
                            }else if(js_data[j].user_id2 == user_id){
                                men_data[i].temp ++;
                                if(num[2]){
                                    if(num[2] >= 0){
                                        men_data[i].user_jf += 5 + num[2];
                                    }else {
                                        men_data[i].user_jf += 5
                                    }
                                }
                            }else if(js_data[j].user_id3 == user_id){
                                men_data[i].temp ++;
                                if(num[3]){
                                    if(num[3] >= 0){
                                        men_data[i].user_jf += 5 + num[3];
                                    }else {
                                        men_data[i].user_jf += 5
                                    }
                                }
                    ...
                    var arr = men_data;
                    arr.sort(function(a,b){return b.user_jf - a.user_jf});
                    console.log('arrs>>',arr);
                    if(iftrue){
                        var arrs =[];
                        if(men_data.length >= 20){
                            if(arr[0] && arr[1] && arr[3]){
                                arrs[0] = arr[0];
                                arrs[1] = arr[1];
                                arrs[2] = arr[2];
                            }
                            var datas = {code:100,msg:arrs};
                            callback(datas);
                         }else{
                             callback({code:103,msg:'该牌友会成员不满20人！'});
                         }
                    }else{
                        var data = {"code": 100,msg:arr};
                        callback(data)
                    }
...
//给牌友会积分前三 发邮件使用；add gl 12/16;
exports.to_send_email_three = function(callback){
    var sql = 'select * from txh_info';
    query(sql,function(err,rows){
        if(!err){
            var txh_datas = rows;
            for (var i = 0; i< txh_datas.length; i++){
                if(txh_datas[i] && txh_datas[i].txh_id){
                    menber_rink(txh_datas[i].txh_id,-1,true,function(data){
                        console.log('menber_rink info datass >>',data);
                        if(data){
                            callback(data);
                            return;
                        }else{
                            callback({code:103,msg:'排名数据为空！不需要发email'});
                            return ;
                        }
                    })
                }
            ...
            var data = {code:101,msg:'查询牌友会id出错'};
            callback(data);
            return;
        }
    })
...
var check_permissions = function (txh_id, user_account, creator_id, callback) {
    callback = callback == null ? nop : callback;
    var sql = 'select user_title from txh_member_table where target_account = "{0}" and txh_id = "{1}"';// 11/13 判断权限添加txh_id
    sql = sql.format(user_account, txh_id)
    query(sql, function (err, rows) {
        if (!err) {
            if (rows && rows.length > 0) {
                    ...
                    if (ret && ret.gems && ret.gems >= 8) {
                        ...
                        try {
                            ...
                            if (is_daili.data.code == 0) {
                                ...
                                if ((parseInt(gems) + parseInt(s_gems)) > ADDGEMS) {
                                    var data5 = {code: 120};
                                    num = 18 - s_gems;
                                    callback(data5);
                                } else {
                            ...
                            email.send_email("牌友会创建房间调用/api/v1/dealers/checkdaili接口出错", "err" + e.stack);
                            return;
                        }
                    }
                } else {
...
var t_change_txh_info = function (req, res, http) {
    ...
    change_txh_info(txh_id, txh_disc, creator_id, txh_name, function (data) {
        ...
        if (data && data.code == 100) {
            ...
            console.log('修改同乡会信息入参错误!');
            http.send(res, 103, '修改同乡会信息入参错误！');
            return;
        } else {
            console.log('未知错误!');
            http.send(res, 10000, '未知错误！');
...
var t_check_permissions = function (req, res, http) {
    ...
    if (txh_id == null || target_account == null || creator_id == null) {
        console.log('判断是否能创建房间入参错误');
        http.send(res, 103, '判断是否能创建房间入参错误');
        return;
    }
...
var t_txh_room_history = function (req, res, http) {
    ...
    var user_id = data.user_id;
    if (txh_id == null || user_id == null) {
        console.log('查询txh历史房间入参错误!');
        http.send(res, 103, '查询txh历史房间入参错误');
        return;
    }
...
var t_user_join_pyh_num = function (req, res, http) {
    ...
    var user_id = data.user_id;
    if (user_id == null || txh_id == null) {
        console.log('查询玩家加入pyh num入参错误!');
        http.send(res, 103, '查询玩家加入pyh num入参错误!');
        return;
    }
...
//在大厅中输入房间号进入房间时，判断房间是不牌友会的房间，是就判断是否为牌友会成员；
// 不是牌友会成员不让进入，通知加入牌友会才能进入；是牌友会成员可以通知从牌友会房间列表进入；
var is_pyh_room = function (roomid, user_id, callback) {
    if (roomid == null || user_id == null) {
        return false;
    }
    var sql = 'select * from txh_room_table where room_id = "{0}" and room_is_excit = 0';
    sql = sql.format(roomid);
    var t_data = pool.query(sql);
    if (t_data.err) {
        return false;
    }
    if (t_data.rows.length > 0) {
        var txh_id = t_data.rows[0].txh_id;
        var sql = 'select * from txh_member_table where txh_id = "{0}" and user_id = "{1}"';
        sql = sql.format(txh_id, user_id);
        var t_user = pool.query(sql);
        if (t_user.err) {
            return false;
        }
        if (t_user.rows.length > 0) {
            callback({code: 117});//是牌友会成员
        } else {
            callback({code: 116});//不是牌友会成员
        }
    } else {
        callback({code: 122});//不是牌友会房间
    }
};
var pd_is_pyh_room = function (req, res, http) {
    var roomid = req.query.roomid;
    var user_id = req.query.user_id;
    if (roomid == null || user_id == null) {
        http.send(res, 103, "判断是否为pyh房间入参错误！");
        return;
    }
    is_pyh_room(roomid, user_id, function (data) {
        if (data && data.code == 117) {
            http.send(res, 117, "该房间是牌友会房间，您是牌友会成员您可以进入该房间！");
            return;
        } else if (data && data.code == 116) {
            http.send(res, 116, "该房间是牌友会房间，您不是该牌友会成员，您不能进入该房间，赶快去加入牌友会吧！");
            return;
        } else if (data && data.code == 122) {
            http.send(res, 122, "该房间不是牌友会房间");
            return;
        }
    });
};
// 积分排名。GuoLin 20171216.
var menber_rink_port = function(req,res,http){
    var txh_id = req.query.txh_id;
    var time = req.query.time_data;
    if(txh_id == null || time == null){
        http.send(res,103,"search menber ji fen pai error！");
        return;
    }
    menber_rink(txh_id,time,false,function(data){
        if(data && data.code == 100){
            http.send(res,100,data.msg);
            return;
        }else{
            http.send(res,101,"查询数据错误");
            return;
...
exports.t_pd_is_pyh_room = pd_is_pyh_room;
exports.menber_rink_port = menber_rink_port;
...
handles.agree_add_gems = function (socket, datas) {
    ...
    if (data1 && data1.code == 1) {
        ...
        try {
            ...
            email.send_email("调用接口/api/v1/dealers/des_gems出错", "err" + e.stack);
            return;
        }
    } else {
        console.log('玩家放弃了充值！')
