...
var myVersion = "nkbhmj V0.01 171104";
//by mc
...
function mylog_debug() {
    ...
    for (var i = 0; i < t; i++) {
        console.log("gamemgr_nkbh", arguments[3 * i], arguments[3 * i + 1], arguments[3 * i + 2]);
    }
    var y = arguments.length % 3;
    if (y != 0) {
        switch (y) {
            case 1: {
                console.log("gamemgr_nkbh", arguments[3 * t]);
                break
            }
            case 2: {
                console.log("gamemgr_nkbh", arguments[3 * t], arguments[3 * t + 1]);
                break;
            }
        }
    }
...
function shuffle(game) {
    ...
    console.log("牌生成了，数量是：", game.mahjongs.length)       //laoli 1023
    ...
    //console.log(">> 配牌1:",mahjongs);

    // NOTE: 启用配牌系统, laoli 171016
    if (enablePeiPai_debugonly) {
        console.log(">> call配牌系统...", game.roomInfo.id);
        var roomId = game.roomInfo.id;
        var mjArry = http.getSync('http://60.205.203.40:1017/get_set_number', {'roomId': roomId});
        // console.log("shu",mjArry.data, mjArry.data.msg[0] , mjArry.data.msg.length > 1,mjArry.data.code==101);
...
function sendOperations(game, seatData, pai) {
    if (seatData == null)     // laoli add 1020 for 23mj
    {
        return;
    }
...
//      玩家的点击动作的优先判断应该都在这里
function doAction(game, seatData, action, data) { //点击界面吃碰杠胡过时调用
    if (!game.actionMap) {
        game.actionMap = {};
    }
    //console.log(">> doAction:", action, seatData.userId, seatData.seatIndex);
    //console.log(">> doAction1 actionmap:", game.actionMap);
    ...
    //console.log(">> doAction2 actionmap:", game.actionMap);
...
function doUserMoPai(game, lastFangGangSeat) {
    ...
    console.log("-----------------------------------------")
    var count = 0; //记录观战玩家数量 mcc1212
    var isMo = false;
    var data = {msg:"剩余三张流局噢 !"};
    if(game.gameSeats.length == 2){   //mc1212
        if((game.mahjongs.length - game.currentIndex) == 71){
            //console.log("剩余70张test")
            userMgr.broacastInRoom('mj_lj_push', data, turnSeat.userId, true);
        }
        if((game.mahjongs.length - game.currentIndex) == 31){
            isMo = true;
        }
    }else if(game.gameSeats.length == 3 || game.gameSeats.length == 4){ //MC1212
        for (var jl = 0 ; jl < game.gameSeats.length; jl++){
            if(game.gameSeats[jl].holds.length == 0){
                count++;
            }
        }
        if(count == 0){
            if(game.mahjongs.length - game.currentIndex == game.conf.mapaixuanze+3){
                userMgr.broacastInRoom('mj_lj_push', data.msg, turnSeat.userId, true);
            }
            if(game.mahjongs.length - game.currentIndex == game.conf.mapaixuanze){
                isMo = true;
            }
        }else if(count == 1 || count == 2){
            if((game.mahjongs.length - game.currentIndex) == 34){
                userMgr.broacastInRoom('mj_lj_push', data.msg, turnSeat.userId, true);
            }
            if((game.mahjongs.length - game.currentIndex) == 31){
                isMo = true;
...
function calculateResult(game, roomInfo) {
    ...
    for (var i = 0; i < gameseatlength; ++i) {  //laoli 1021
        ...
        //胡牌分 + 马分
        var baseScore = game.conf.baseScore; //difen
        //console.log("--------------")
        //console.log("sd.huInfo:", sd.userId, sd.huInfo)
        for (var j = 0; j < sd.huInfo.length; ++j) {
            ...
            if (info.isGangHua || info.isDianGangHua) { //todo,点杠，补杠杠花 x2
                fanBei *= 2;
            }
            if (info.isQiangGangHu) {
                fanBei *= 1;
            }
            if(info.isTianHu) fanBei=16;    // TODO , 天胡就是16倍
...
function doGameOver(roomInfo, forceEnd) {
    ...
    if (game) {
        ...
        for (var i = 0; i < game.currentUsercnt; ++i) {       //laoli 1020
            ...
            dbresult[i] = sd.score;
            //console.log("dbresult[i]:",dbresult[i])
...
exports.sync = function (userId) {
    ...
    //同步整个信息给客户端
    //console.log(">>do samge sync:",seatData.userId, data)
...
exports.lateJoin = function (roomId, userId) {
    ...
    var roomInfo = roomMgr.getRoom(roomId);
    console.log(roomInfo.game.gameSeats.length)
    console.log("================================")
...
exports.begin = function (roomId) {
    ...
    for (var i = 0; i < getMaxuserCount(); ++i) {
        ...
        // data.hued = false;
        // data.actions = [];
        //data.iszimo = false;   //自摸
        //data.isGangHu = false; //杠上花
        //data.score = 0;
        //data.huInfo = [];
        //data.genZhuangFen = 0; //跟庄分数
        //data.anGangScore = 0;  // 暗杠分数
        //data.mingGangScore = 0;  // 明杠分数
        //data.huPaiScore = 0;    // 胡牌分
        //data.lastFangGangSeat = -1;
        //统计信息
        //data.numZiMo = 0;
        //data.numAnGang = 0;
...
function notifyChuPai(game) {
    ...
    console.log(">>turn TO", turnSeat.userId, turnSeat.holds)
    game.state = "playing";
...
exports.chuPai = function (userId, pai) {
    ...
    for (var i = 0; i < game.currentUsercnt; ++i) {
        ...
        // console.log(">>出牌是，下家是", seatIndex, nextturn)
        if (nextturn == i) {
            //console.log(ddd.canHu, ddd.canGang, ddd.canPeng)
    ...
    if (!hasActions) {
        ...
        // if(seatData.folds.length > 23){
        //     doGameOver(game.roomInfo)
        // }
        game.chuPai = -1;
        moveToNextUser(game);
        // var cout = 0
        // if (game.gameSeats.length == 2 && (game.mahjongs.length - game.currentIndex) > 30){ //mc1128
        //     doUserMoPai(game);
        // }
        // else if(game.gameSeats.length == 3 || game.gameSeats.length == 4){
        //     for(var kk = 0 ; kk< game.gameSeats.length; kk++){
        //         if(game.gameSeats[kk].holds.length == 0 &&  (game.mahjongs.length - game.currentIndex) > 30){
        //             doUserMoPai(game);
        //         }else if (game.gameSeats[kk].holds.length != 0){
        //             cout++
        //         }
        //     }
        //     if(cout == 3 || cout == 4){
...
function doPeng(game, seatData, data) {
    ...
    checkCanWanGang(game,seatData)   //mc1109fixed
    //通知玩家做对应操作
    sendOperations(game, seatData, game.chuPai);  //mc1109fixed
    //广播通知玩家出牌方
...
function doGang(game, turnSeat, seatData, gangtype, numOfCnt, pai) {
    ...
    seatData.guoHuFan = -1;
    //var isZhuanShouGang = false; //mc1110fixed
    if (gangtype == "wangang") { //mc1110fixed
        ...
        //if (seatData.holds[seatData.holds.length - 1] != pai) {  //mc1110fixed
...
function doChi(game, seatData, data) {
    mylog_debug("执行吃：", seatData.userId, data)
    ...
    // console.log(seatData.holds, seatData.countMap);
    ...
    checkCanWanGang(game,seatData)   //mc1113fixed
    checkCanAnGang(game,seatData)    //mc1113fixed
    //通知玩家做对应操作
    sendOperations(game, seatData, game.chuPai);  //mc1113fixed
    //广播通知其它玩家
...
showGameOverResult1 = function (game, results) {
    mylog_debug("**** 算分结果 ****", game.roomInfo.id);
    mylog_debug(new Date());
    mylog_debug("*** 马牌 ***");
    mylog_debug(game.mas);
    for (var i = 0; i < game.currentUsercnt; ++i) {
        var s = game.gameSeats[i];
        var userid = s.userId;
        var huinfo = s.huInfo;
        mylog_debug(util.format("**%d,玩家%d**", i + 1, userid));
        if (huinfo) {
            mylog_debug("  胡牌：", huinfo[0]);
        }
        ...
        mylog_debug("  手牌：" + holdstr);
        mylog_debug("  桌牌：" + foldstr);
        mylog_debug("  吃碰杠牌：" + cpgstr);
        // var jingstr = "  精分：";
    ...
    // mylog_debug("*** 精原始Info ***");
    // for (var j = 0; j < game.currentUsercnt; ++j) {
    //     mylog_debug(j, game.gameSeats[j].jingsInfo);
    // }
    mylog_debug("");
    //mylog_debug("*** 下发信息 ***");
    //mylog_debug(results)
