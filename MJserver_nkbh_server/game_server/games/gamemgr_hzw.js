...
function mylog_debug() {
    ...
    for (var i = 0; i < t; i++) {
        console.log("gamemgr_hzw", arguments[3 * i], arguments[3 * i + 1], arguments[3 * i + 2]);
    }
    var y = arguments.length % 3;
    if (y != 0) {
        switch (y) {
            case 1: {
                console.log("gamemgr_hzw", arguments[3 * t]);
                break
            }
            case 2: {
                console.log("gamemgr_hzw", arguments[3 * t], arguments[3 * t + 1]);
                break;
            }
        }
    }
...
function deal(game) {
    ...
    for (var i = 0; i < 13 * game.currentUsercnt; ++i) {
        ...
        huabu(paiMo); //发牌时判断摸的牌是否是花牌
        function huabu(paiMo) {
            if(isHua(paiMo)){
                buhua(game,turnSeat,paiMo);
                paiMo = mopai(game, seatIndex);
                //通知房间里对应的玩家补花
                userMgr.broacastInRoom('game_buhua_push',{si:i,huamap:turnSeat.huaMap},turnSeat.userId,true);
                huabu(paiMo);
            }
        }
        seatIndex++;
    ...
    //庄家多摸最后一张
    var paiMo1 = mopai(game, game.button);
    huabu(paiMo1);
    //当前轮设置为庄家
