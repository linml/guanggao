...
exports.createRoom = function (creator, roomConf, gems, ip, port) {
     ...
     */
    console.log("roomConf:111",roomConf)
...
exports.forceBegin = function (userId, value) {
    ...
    //var s = room.seats[seatIndex];        //laoli
    if (room.game == null) {
        var curSeat = null;
        var readycnt = 0;
        var seatcnt = 0;
        var bigestseatindex = 0;
        for (var i = 0; i < room.seats.length; ++i) {
            var ss = room.seats[i];
            if (ss.userId > 0) {
                seatcnt++;
        ...
        if (seatcnt < 2) {
            console.log(">>forceBegin2...人不够，至少2人", userId, readycnt);
            var data1 = {};
        ...
        if (seatcnt != readycnt) {
            console.log(">>forceBegin3...有人未准备好", userId, readycnt);
            var data2 = {};
            data2.msg = "有人未准备好。再等一下吧。";
            userMgr.sendMsg(userId, 'game_alert_msg', data2);
            return 0;
        }
        if (seatcnt != bigestseatindex + 1) {
            console.log(">>forceBegin4...玩家之间有空位", userId, readycnt);
            var data3 = {};
            data3.msg = "玩家之间有空位，无法开始。\n试一试退出再进房吧。";
